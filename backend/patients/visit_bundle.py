...
from consultation.session_bundle import _paginated_payload


def build_visit_workspace_bundle(visit: Visit, *, vitals_limit: int = 10) -> dict:
    """Fetch diagnoses, orders, prescriptions, and vitals scoped to one visit."""
    visit_id = visit.id
    patient_id = visit.patient_id

    diagnoses_qs = (
        Diagnosis.objects.filter(visit_id=visit_id)
    ...
    prescriptions_qs = Prescription.objects.filter(visit_id=visit_id)
    if patient_id:
    ...
    lab_qs = LabOrder.objects.filter(visit_id=visit_id)
    if patient_id:
    ...
    radiology_qs = RadiologyOrder.objects.filter(visit_id=visit_id)
    if patient_id:
    ...
    nursing_qs = NursingOrder.objects.filter(visit_id=visit_id)
    if patient_id:
    ...
    physio_qs = PhysioOrder.objects.filter(visit_id=visit_id)
    if patient_id:
    ...
    eye_qs = EyeOrder.objects.filter(visit_id=visit_id)
    if patient_id:
    ...
    vitals_qs = (
        ...
        .select_related("patient", "visit", "recorded_by", "visit__location_clinic")
