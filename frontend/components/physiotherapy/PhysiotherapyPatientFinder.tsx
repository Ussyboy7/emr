...
import {
  buildPhysioCompletedHref,
  buildPhysioOrdersHref,
...
export function PhysiotherapyPatientFinder() {
  ...
  const runSearch = useCallback(async (term: string) => {
    ...
    setSearched(true);
    try {
      const res = await physioService.getPatientTracker(q);
      setHits(res.results || []);
    } catch {
      setHits([]);
    } finally {
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-lg flex items-center gap-2">
          ...
          Find patient in physiotherapy
        </CardTitle>
        <p className="text-sm text-muted-foreground">
          Search by patient name, patient ID, or Physio ID (e.g. PHY-000002) — see where they are in the workflow.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="relative">
          ...
          <Input
            placeholder="Patient name, ID, Physio ID (e.g. PHY-000002)…"
            value={query}
        ...
        {!loading && searched && searchTerm.length >= 2 && hits.length === 0 && (
          <p className="text-sm text-muted-foreground py-2">No physiotherapy records found for this search.</p>
        )}

        {!loading && activeHits.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Active in physio</p>
            {activeHits.map((hit, idx) => (
              ...
              >
                ...
                <Button variant="outline" size="sm" className="shrink-0" asChild>
                  <Link href={buildPhysioOrdersHref(searchTerm, hit.tab as PhysioOrdersTab)}>
                    Open
            ...
            {doneHits.slice(0, 8).map((hit, idx) => (
              ...
              >
                ...
                <Button variant="ghost" size="sm" className="shrink-0" asChild>
                  <Link href={buildPhysioCompletedHref(searchTerm)}>
                    Completed Sessions
                    <ArrowRight className="h-3.5 w-3.5 ml-1" />
                  </Link>
                </Button>
