...
export default function EyecareAnalyticsPage() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await eyecareService.getAnalyticsSummary(r.start, r.end);
      setData(res);
    } catch (e: unknown) {
      console.error(e);
  ...
  const exportCsv = () => {
    ...
    const csv = eyecareAnalyticsToCsv(data, viewMode, year, startDate, endDate);
    const period = viewMode === 'year' ? year : `${startDate}_to_${endDate}`;
    triggerCsvDownload(`eyecare_analytics_${period}.csv`, csv);
